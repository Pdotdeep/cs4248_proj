    ...
    for epoch_curr in range(n_iters):
        ...
        for iter in range(len(training_pairs)):
            ...
            if iter % print_every == 0:
                ...
                torch.save(encoder.state_dict(), "encoder_no_attn.pth")
                torch.save(decoder.state_dict(), "decoder_no_attn.pth")
...
decoder1 = DecoderRNN(hidden_size, VOCAB_MODEL.n_words).to(device)
print("COMMENSING TRAINING")
trainIters(encoder1, decoder1, 100, print_every=100)


#UNCOMMENT TO COMMENSE TRAINING  FROM CHECKPOINT
# encoder1.load_state_dict(torch.load('encoder_no_attn_3_.pth'))
# decoder1.load_state_dict(torch.load('decoder_no_attn_3_.pth'))
