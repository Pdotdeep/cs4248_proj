...
def prepareData():
    ...
    lang_model = LanguageModel("english")
    for pair in pairs:
...
class AttnDecoderRNN(nn.Module):
    ...
    def forward(self, input, hidden, c_state, encoder_outputs):
        embedded = self.embedding(input).view(1, 1, -1)
        ...
        output = F.relu(output)
        output, (hidden,c_state) = self.gru(output, (hidden,c_state))
...
def train(input_tensor, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, max_length=MAX_LENGTH):
    ...
    c_state = torch.zeros_like(encoder_hidden)
    use_teacher_forcing = True if random.random() < teacher_forcing_ratio else False
    ...
    if use_teacher_forcing:
        ...
        for di in range(target_length):
            decoder_output, decoder_hidden, decoder_attention = decoder(
                decoder_input, decoder_hidden, c_state, encoder_outputs)
            loss += criterion(decoder_output, target_tensor[di])
            decoder_input = target_tensor[di]  # Teacher forcing
...
def trainIters(encoder, decoder, n_iters, print_every=1000, plot_every=100, learning_rate=0.01):
    ...
    for iter in range(1, n_iters + 1):
        ...
        if iter % print_every == 0:
            ...
            torch.save(encoder.state_dict(), "encoder_lstm_attn_collated.pth")
            torch.save(decoder.state_dict(), "decoder_lstm_attn_collated.pth")
...
#attn_decoder1 = DecoderRNN(hidden_size, lang_model.n_words).to(device)
encoder1.load_state_dict(torch.load('encoder_lstm_attn_collated.pth'))
attn_decoder1.load_state_dict(torch.load('decoder_lstm_attn_collated.pth'))
