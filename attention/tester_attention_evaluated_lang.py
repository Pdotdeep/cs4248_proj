...
attn_decoder2 = AttnDecoderRNN(hidden_size, lang_model.n_words - 8, dropout_p=0.1).to(device) # todo: manual fix 
#attn_decoder2 = DecoderRNN(hidden_size, lang_model.n_words).to(device)
encoder2.load_state_dict(torch.load('encoder_attn_collated.pth', map_location=torch.device('cpu')))
attn_decoder2.load_state_dict(torch.load('decoder_attn_collated.pth', map_location=torch.device('cpu')))
